...
public class ProfileHandler extends BaseHandler {
    ...
    private void handleGetProfile(HttpExchange exchange, User user) throws Exception {
        // Fetch fresh from DB to get latest verification statuses
    ...
    private void handleUpdateUsername(HttpExchange exchange, User user, String body) throws Exception {
        ...
        if (newUsername == null || newUsername.trim().isEmpty()) {
            sendResponse(exchange, 400, JsonUtil.error("New username is required"));
            return;
        }
        ...
        if (err != null) {
            sendResponse(exchange, 400, JsonUtil.error(err));
            return;
        }

        // Same as current?
        if (newUsername.equalsIgnoreCase(user.getUsername())) {
            sendResponse(exchange, 400, JsonUtil.error("This is already your username"));
            return;
        }

        // Check availability
        if (userDAO.existsByUsername(newUsername)) {
            sendResponse(exchange, 409, JsonUtil.error("Username '" + newUsername + "' is already taken"));
            return;
        }
    ...
    // Step 1: Sends OTP to the NEW email
    // Body: { "newEmail": "..." }
    // ---------------------------------------------------------------
    private void handleInitiateEmailChange(HttpExchange exchange, User user, String body) throws Exception {
        ...
        if (newEmail == null || newEmail.trim().isEmpty()) {
            sendResponse(exchange, 400, JsonUtil.error("New email is required"));
            return;
        }
        ...
        if (newEmail.equalsIgnoreCase(user.getEmail())) {
            sendResponse(exchange, 400, JsonUtil.error("This is already your email address"));
            return;
        }
        ...
        // Key pattern: "profile:email:change:<userId>:<newEmail>"
    ...
    // Step 2: Verifies OTP and updates email in DB
    // Body: { "newEmail": "...", "otp": "..." }
    // ---------------------------------------------------------------
    private void handleVerifyNewEmail(HttpExchange exchange, User user, String body) throws Exception {
        ...
        if (newEmail == null || otp == null) {
            sendResponse(exchange, 400, JsonUtil.error("newEmail and otp are required"));
            return;
        }
        newEmail = newEmail.trim().toLowerCase();
    ...
    // Step 1: Sends OTP to the NEW phone number
    // Body: { "newPhone": "..." }
    // ---------------------------------------------------------------
    private void handleInitiatePhoneChange(HttpExchange exchange, User user, String body) throws Exception {
        ...
        if (newPhone == null || newPhone.trim().isEmpty()) {
            sendResponse(exchange, 400, JsonUtil.error("New phone number is required"));
            return;
        }
        ...
        if (newPhone.equals(currentClean)) {
            sendResponse(exchange, 400, JsonUtil.error("This is already your phone number"));
            return;
        }
    ...
    // ---------------------------------------------------------------
    // POST /api/profile/verify-new-phone
    // Step 2: Verifies OTP and updates phone in DB
    // Body: { "newPhone": "...", "otp": "..." }
    // ---------------------------------------------------------------
    private void handleVerifyNewPhone(HttpExchange exchange, User user, String body) throws Exception {
        ...
        if (newPhone == null || otp == null) {
            sendResponse(exchange, 400, JsonUtil.error("newPhone and otp are required"));
            return;
        }
        newPhone = newPhone.trim().replaceAll("[\\s-]", "");
    ...
    // Permanently deactivates the account (soft delete)
